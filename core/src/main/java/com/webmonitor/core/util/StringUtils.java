...
public abstract class StringUtils {
	...
	* */
	...
	//将数字格式化成字符串
	public static String valueOf(double dValue)
	{
		return valueOf(dValue, 3, true);
	}
	public static String valueOf(double dValue, int dDigit)
	{
		return valueOf(dValue, dDigit, true);
	}
	public static String valueOf(double dValue, int dDigit, boolean bSimple)
	{
		String strFormat = "%."+ String.format("%d", dDigit) + "f";
		if (bSimple){
			return valueOf(strFormat, dValue);
		}else {
			return format(strFormat, dValue);
		}
	}
	//字符串去除小数点后面的0
	public static String valueOf(String format, Object... args)
	{
		String strValue = format(format, args);
		if (strValue.indexOf('.') > 0) {
			int length = strValue.length();
			while (length > 0 && strValue.charAt(length - 1) == '0') {
				length--;
			}
			if (strValue.charAt(length - 1) == '.')
				length--;
	...
	/// <summary>
	/// 获取指定字符分割的基于Zero_index的索引
	/// </summary>
	/// <param name="strResource">分割的源字符串</param>
	/// <param name="iSub">获取的索引号，以零开头</param>
	/// <param name="ch">分隔字符符</param>
	/// <returns>若源为null则返回""，若找不到第iSubstring+1个分隔符，则返回""</returns>
	public static String subString(String strResource, int iSub, String ch)
	{
		if (strResource == null || iSub < 0)
			return "";
		String[] strArray = strResource.trim().split(ch);
		if (strArray == null || strArray.length  <= iSub)
			return "";
		return strArray[iSub];
	}
