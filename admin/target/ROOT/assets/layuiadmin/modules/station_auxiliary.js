layui.define(['form', 'drawer', 'table','station_auxiliary_func','station_func'], function (exports) {
    ...
    form.on('submit(addSensor)',function (data) {
        ...
        let vender=sencompanylist.getValue();
        jsondata.vender=vender[0].sensorVender;
        let stringtest=JSON.stringify(jsondata);
        $.ajax({
    ...
    form.on('select(type)',function (data) {
        ...
        getsencompanylist(type);
        form.render("select");
    })

    function getsencompanylist(type){
        $.ajax({
            url:'/devicelist/getSensorCompanyList',
            data:{
                type:type
            },
            success:function (res) {
                ...
                //
                //     let options="<option value="+data[j].sensorVender+">"+data[j].venderName+"</option>";
                //     $("#sensor_vender").append(options);
                //     $("#sensor_cmd").val(data[j].sensorCmd);
                // }
                sencompanylist = xmSelect.render({
                    el: '#sencompanylist',
                    radio: true,
                    clickClose: true,
                    layVerify: 'required',
                    ...
                    on:function (data) {
                        ...
                        $("#sensor_cmd").val(change[0].sensorCmd);
                    }
                })
