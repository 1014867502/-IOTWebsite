layui.define(['form', 'drawer', 'table', 'upload','layer','message'], function (exports) {
    var $ = layui.$
        , setter = layui.setter
        , admin = layui.admin
        , table = layui.table
    ...
    function getExpireCount() {
        let bool=parent.msgboolen;
        let agentnum=parent.agentnum;
        if(bool==1&&agentnum=="1"){//既是超管，也需要是本公司的的用户
            $.ajax({
                url:'/borrow/countExpire',
                async:false,
                success:function (res) {
                    let devices=res.data
                    let msg=message.initMessage();
                    if(devices.closeexpire>0){
                        message.sendMessage(msg,"warning","临近到期的借出设备有"+devices.closeexpire+"台","警告");
                    }
                    if(devices.outexpire>0){
                        message.sendMessage(msg,"error","到期的借出设备有"+devices.outexpire+"台","过期");
                    }
                    parent.msgboolen=2;
                }
            });
    ...
    $("#add_movedevice").on('click', function () {
        uploadmoveopen();
    });
            ...
            success: function (data) {
                switch (data.data) {
                    case "user":
                        ...
                        $("#add_movedevice").css("display","none");
            ...
            success: function (data) {
                switch(type){
                    case 0:
                        ...
                        break;
                    case 1:
                        loadcompanylist(data.data,"#companylist");//弹出窗口内的公司列表
                        break;
                    case 2:
                        loadcompanylist(data.data,"#agentslist");//弹出窗口内的公司列表
                        break;
                }
    ...
    function uploadmoveopen() {
        ...
        layer.open({
            ...
            , resize: false
            , shade: 0.3
            , id: 'LAY_move' //设定一个id，防止重复弹出
            //,btnAlign: 'c'
            , content: $('#moveswindow')
            , success: function (index,layero) {
                excutemoveupload();
                getCompanyListByRole(userid,2);
                layerindex2=index;
            },
            cancel: function(index, layero){
                layer.confirm('确实要关闭么?', {icon: 3, title:'提示'}, function(index1){
            ...
            , end: function () {
                ...
                once = true;
                deleteupload();
                element.progress("sqlmoveprogress","0%");
                $("#upload_moveerror").html("");
                $("#sqlmoveprogress").css("display","none");
                if(curprogress!=100){
                    //stopupload();
                }
            }
    ...
    /**上传方法**/
    function excutemoveupload() {
        let demoListView = $("#moveList");
        $("#uploadmove").append("<button type=\"button\" class=\"layui-btn  layui-btn-sm layui-btn-normal\" id=\"testList\" style=\"margin-top: 6px;margin-left: 30px;\">选择数据文件</button>");
        let url = "/devicelist/fileupload?userid="+userid;
        var uploadlistins = upload.render({
            elem: '#testList', // 文件选择
            acceptMime: '.csv',
            ...
            choose: function (obj) {
                ...
                if (columnlist.length > 1) {
                    layer.msg("文件列数大于1，不符合格式，请修改文件");
                    return;
                }
                if (once) {
                    once = false;
                    let type=companylist.getValue();
                    if (type[0].value==null||type[0].value=="") {
                        layer.msg("请选择迁移公司");
                        return;
                    }
                    $("#columwindow").css("display", "block");
                    delete this.files[index];
                    $.ajax({
                        url:'/devicelist/movefileinput',
                        data:{
                            // json:jsondata,
                            filename:filename,
                            userid:userid,
                            agentnum:type[0].value
                        },
                        async:false,
                        success:function (data) {
                            if(data.data=="文件上传成功！"){
                                ...
                                inputresult="error";
                                layer.msg(data.data);
                                $("#upload_moveerror").html("警告:"+data.data+" 执行已中止！");
                                $("#upload_moveprogress").removeClass("layui-bg-blue");
                                $("#upload_moveprogress").addClass("layui-bg-red");
                                window.clearInterval(progressfinish);
                            }
                        }
                    });
                    $("#sqlmoveprogress").css("display","block");
                    element.progress("sqlmoveprogress","0%")
                    progressfinish=setInterval(function(){
                        $.ajax({
                            url:'/devicelist/progressuploadfile',
                            ...
                            success:function (data) {
                                ...
                                element.progress("sqlmoveprogress",data.data+"%")
                            }
                        })},300);
                    //     }
                    // });
