layui.define(['form','drawer','table','laydate','layer','Global_variable','tree'], function (exports) {
    ...
    form.on('select(type2)',function(data){
        ...
        if(type=="0"){
            ...
            loadcompany(agentNumber,"superadmin");
            loadcomauthority(agentNumber);
        }
        else if(type=="3"){
            document.getElementById("editwebright").innerHTML="";
                    ...
                    if(data.select!=null&&data.select!=""){
                        ...
                        for(let p in data){
                            ...
                            if(p.indexOf("webauthor_")>-1){
                                let length=p.length;
                                if(length<12){
                                    let num=parseInt(p.substring(length-1,length));
                                    if(num!=0){
                                        num=num+4;
                                    }
                                    comweb+=num+"@";
                            ...
                            else if(p.indexOf("layuiTreeCheck_")>-1&&p.indexOf("undefined")<=0){
                                let length=p.length;
                    ...
                    $.ajax({
                        ...
                        success:function(res){
                            ...
                            for(let i=0;i<allweb.length;i++){
                                ...
                                if(allweb[i].value>=1&&allweb[i].value<=4){
                                    tree.setChecked('demoId',parseInt(allweb[i].value));
                                }
                            }
            ...
            $.ajax({
                  ...
                  success:function(res){
                      ...
                      for(let i=0;i<web.length;i++){
                          webselect+="<input  lay-skin=\"primary\" type=\"checkbox\" id='web_"+web[i]+"' name=\"webauthor_"+i+"\"  title="+allweb[web[i]].name+">"
                          // if(i>=1&&i<=4){
                          //     tree.setChecked('demoId',i);
                          // }
                      }
                      for(let j=0;j<allapp.length;j++){
                          appselect+="<input  lay-skin=\"primary\" type=\"checkbox\" id='app_"+j+"' name=\"appauthor_"+j+"\"  title="+allapp[j].name+">"
                      }
    ...
    function getcompanylist(type){
        $.ajax({
            ...
            success: function(data){
                if(type!=null){
                    initcompanylist(data.data,type);
                }else{
    ...
    /**获取公司列表(编辑页面初始化)**/
    function loadcompany(init,type){
        $.ajax({
            url:'/company/getAllCompany',
            async:false,
            success: function(data){
                if(type!=null){
                    loadcompanylist(data.data,init,type);
                }else{
    ...
    /**加载公司列表(新增用户)**/
    function initcompanylist(json,type){
        var arrData = [];
        var selectSn ="";
        if(type!="superadmin"){
            for(let i=0;i<json.length;i++){
    ...
    /**加载公司列表(编辑页面初始化)**/
    function loadcompanylist(json,init,type){
        var arrData = [];
        var selectSn ="";
        if(type!="superadmin"){
            for(var i=0;i<json.length;i++){
    ...
    function loadprojectlist(json){
        var arrData = [{name:"无",value:"0"},{name:"所有项目",value: "all"}];
        var selectSn ="";
        for(var i=0;i<json.length;i++){
            var item = json[i];
            var jsonStr = {};
        ...
        if(document.getElementById("projectlist")!=null) {
            projectlist = xmSelect.render({
                ...
                model: {
                    ...
                    if(isAdd) {
                        if (arr.length == json.length+2) {
                            if (change[0].value == "0"&&arr[0].value!="0") {
                                arr.splice(0,arr.length-1);
                            }
                            else if(change[1].value == "all"&&arr[1].value!="all"){
                            ...
                            else if(change.length>0&&change[0].value == "all"){
                                arr.splice(0,arr.length-1);
                            }
                            else{
    ...
    function initloadprojectlist(json,init){
        var arrData = [{name:"无",mutex:1,value:"0"},{name:"所有项目",mutex: 2,value: "all"}];
