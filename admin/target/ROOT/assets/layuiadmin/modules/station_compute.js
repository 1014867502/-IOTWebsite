layui.define(['element', 'form', 'drawer', 'table', 'station_compute_func','station_func'], function (exports) {
    ...
    form.on('radio(mark)', function (data) {
        ...
        if (beforemark !=  mark) {
            if (mark == 1) {
                ...
                computefunc.altermark(mark.toString());
                computefunc.computemark(baselat, baselon, baseheight, mark);
                basex = $("#ecefx").val();
                basey = $("#ecefy").val();
                basez = $("#ecefz").val();
                beforemark = 1;
            ...
            success: function (data) {
                ...
                if (device.rawName!=null) {
                    ...
                    if (device.secondBase != "0") {
                        ...
                        if (document.getElementById("secondbase") != null) {
                            document.getElementById("secondbase").innerHTML = "";
                        }
                        let ntripbase = device.ntrIpBase.toString();
                        if (ntripbase == "1") {
    ...
    function getcurlocal(sn) {
        new Promise((resolve, reject) => {
            baselon = sendorder(sn, "GET,GPS.POSITION.LON");
            baselat = sendorder(sn, "GET,GPS.POSITION.LAT");
            baseheight = sendorder(sn, "GET,GPS.POSITION.HEIGHT");
            $("#baseLat").val(baselat);
            $("#baseLon").val(baselon);
            $("#baseHeight").val(baseheight);
            resolve();
