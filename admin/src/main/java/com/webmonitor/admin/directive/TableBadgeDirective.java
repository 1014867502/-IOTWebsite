 ...
 *
 * 调用方式:
 * 方式1: #tableBadge("{name:'ds',dictObj:{table:'sys_dict',object:'sys_object',field:'data_source'}}")
 * 方式2: #tableBadge("{name:'type',dataSets:[{name:'Table',value:'table'},{name:'View',value:'view'}]}")
 * 方式3: #tableBadge("{name:'type',sql:'select name, value from Tools'}")
 */
public class TableBadgeDirective extends Directive {
        ...
        try {
            ...
            if (!Tools.isEmpty(dataSetList)) {
                for (CustomDataSet set : dataSetList) {
                    ...
                    if (!Tools.isEmpty(obj.getClassNameCondition())
                            && set.getValue().equals(obj.getClassNameCondition())
                            && !Tools.isEmpty(obj.getClassName())) {
                        className = obj.getClassName();
                    } else {
                        className = "";
                    }
                    result.append("<span class='layui-badge "+className+"'>"+set.getName()+"</span>");
                    result.append("{{# } }}");
                }
            }
            if(!Tools.isEmpty(obj.getSql()) || !Tools.isEmpty(obj.getDictObj())){
                ...
                for (Record record : recordList) {
                    ...
                    if (!Tools.isEmpty(obj.getClassNameCondition())
                            && value.equals(obj.getClassNameCondition())
                            && !Tools.isEmpty(obj.getClassName())) {
                        className = obj.getClassName();
                    } else {
                        className = "";
                    }
                    result.append("<span class='layui-badge "+className+"'>"+name+"</span>");
                    result.append("{{# } }}");
                }
            }
            result.append("{{# }); }}");
