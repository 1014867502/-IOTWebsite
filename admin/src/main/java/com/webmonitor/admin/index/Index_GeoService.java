...
public class Index_GeoService {
    public static final Index_GeoService me = new Index_GeoService();
    // 存放登录用户的 cacheName
...
//        String hashedPass = HashKit.sha256(salt + password);
    ...
    public Account getGeoAccount(String userid){
        String sql = "SELECT * from tableuserinfo where UserID='"+userid+"'";
        ...
        if (!Tools.isEmpty(record)){
            ...
            account.setID(0);
            account.setPassword(record.getStr("Password"));
            return  account;
        }
        return null;
    }
